...
description: "Transform a user-supplied photo into a Gathered Scenes Zine poster that preserves the source aspect ratio by default, anchors truthful photography inside a spacious source-derived abstract illustration field, turns every supporting fragment into a low-detail visual echo traceable to the source rather than unrelated decoration, integrates one high-chroma hue as compositional structure, and preserves a visibly hand-torn fibrous photo-to-paper edge. Use when the user wants a tactile minimal paper collage with simplified illustration, active negative space, restrained English-default, Chinese, or bilingual micro-text, and an unmistakable but quiet torn-paper boundary."
---
...
2. Preserve the source image's aspect ratio by default unless the user explicitly requests another ratio.
3. Keep the photographic portion truthful.
4. Make every supporting fragment a low-detail visual echo of an identifiable source region, contour, rhythm, color field, or spatial relationship.
5. Simplify complex organic or repetitive detail into a few large legible masses and directional gestures.
6. Turn the illustration into a large designed field rather than a literal copy or small peripheral echo.
7. Build photography, illustration, and one added hue on the same source-derived compositional skeleton.
8. Use the added hue as chromatic structure that changes balance, movement, figure–ground, or meaning.
9. Preserve substantial quiet space inside and around the enlarged illustration.
10. Preserve a tactile hand-torn photo edge at the primary material handoff.
...
1. **Canvas:** source aspect ratio by default, or the user's explicitly requested ratio; paper surface, flat scan, and absence of mockup framing.
...
### Make Supporting Fragments Source-Derived Echoes

Treat every auxiliary fragment, mark, cutout, underprint, contour, and texture passage as a low-detail visual echo of the supplied photograph.

- Derive each fragment from one visible source region, contour, axis, rhythm, gap, silhouette, material, shadow, or color field.
- Preserve only the fragment's defining direction, proportion, spacing, or tonal relationship; remove most descriptive detail.
- Make the fragment touch, align with, continue, overlap, or emerge from its source counterpart or the main torn boundary whenever composition permits.
- Keep the fragment subordinate to the truthful photographic anchor and primary illustration field.
- Before generation, name the source counterpart for every supporting fragment. Remove any fragment whose source cannot be identified in one short phrase.

Do not add generic circles, rectangles, dots, arrows, labels, stamps, botanical marks, paper scraps, or abstract motifs merely to make the poster feel designed. A simple form is allowed only when its contour, placement, rhythm, or color relationship is visibly traceable to the photograph.
...
- Preserve the supplied photograph's aspect ratio by default. Use another ratio only when the user explicitly requests it.
...
1. Inspect the supplied photo and record its dimensions and aspect ratio; use that aspect ratio for the output unless the user explicitly requests another.
2. Build the Scene Card and identify the semantic minimum.
3. Choose the focal area and intended eye path.
4. Extract one or two source shapes that can organize photography, illustration, and color together; assign every planned supporting fragment a specific visible source counterpart.
...
- **Unrelated supporting fragments:** remove every fragment without a named visible source counterpart; rebuild only low-detail echoes that align, touch, continue, overlap, or emerge from the corresponding photo region or torn boundary.
...
- Does the output preserve the supplied photograph's aspect ratio unless the user explicitly requested another?
...
- Can every supporting fragment be traced to a named visible region, contour, rhythm, color field, or spatial relationship in the supplied photograph?
- Does each supporting fragment read as a simplified paper echo that aligns, touches, continues, overlaps, or emerges from its source counterpart rather than as unrelated decoration?
